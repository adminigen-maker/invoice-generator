...
export default async function InvoicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: inv }, canPost, canPay, canCancel, canCredit, { data: creditLines }] = await Promise.all([
    supabase.from("invoice").select(`
      *,
      ...
      allocations:payment_allocation(amount_allocated, payment:payment(id, number, payment_date, method, reference)),
  ...
  const so = inv.sales_order as { id?: string; number?: string } | null;
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {so?.id ? (
              <>
                From sales order{" "}
                <Link href={`/sales-orders/${so.id}`} className="text-blue-600 hover:text-blue-700">{so.number}</Link>
              </>
            ) : "Standalone invoice"}
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          {/* Printing is only meaningful once the invoice is final. */}
          {posted && <PdfButton url={`/invoices/${id}/pdf`} filename={`${inv.number}.pdf`} />}
              ...
              {lines.map((l, i) => (
                <TableRow key={i}>
                  ...
                  <TableCell className="text-right font-mono">
                    {(Number(l.quantity) - (creditedByLine.get(l.id) ?? 0)).toFixed(0)} {l.uom?.code}
                    {(creditedByLine.get(l.id) ?? 0) > 0 && (
                      <div className="text-[11px] text-amber-600 font-sans">
                        {(creditedByLine.get(l.id) ?? 0).toFixed(0)} returned
                      </div>
                    )}
                  </TableCell>
                ...
                {allocations.map((a, i) => (
                  <TableRow key={i}>
                    <TableCell className="font-mono text-xs">
                      {a.payment?.id ? (
                        <Link href={`/payments/${a.payment.id}`} className="text-blue-600 hover:text-blue-700">{a.payment.number}</Link>
                      ) : (a.payment?.number ?? "—")}
                    </TableCell>
...
type AllocRow = {
  ...
  payment?: { id?: string; number?: string; payment_date?: string; method?: string; reference?: string } | null;
};
